...
if __name__=='__main__':
    ...
    #     for m in mm:
    #         for i in lag:
    #             data_df[str(cols[m-1])+'_{}'.format(i)] = data_df.iloc[n-i,m]
    # lag = [1]
    # for m in mm:
    #     for i in lag:
    #         col = data_df.iloc[:,m].values
    #         col_new = np.insert(col,i-1,0) # add a zero
    #         col_new = np.delete(col_new,-1) # remove last element
    #         data_df[str(cols[m])+'_{}'.format(i)] = col_new
    ...
    pickle.dump(data_df, open( 'aspen_data_lag3.p', 'wb'))
    ...
    #
    # degree = 4
    # pipe_gbr = make_pipeline(PolynomialFeatures(degree), gbr)
    ...
    #
    # score = cross_val_score(pipe_gbr,X_train,y_train,cv=10)
    # print('gbr poly training score = {:0.3f}'.format(np.mean(score)))
    #
    # preds_pipe = pipe_gbr.predict(X_test)
    # rmse = np.sqrt(np.sum((y_test - preds_pipe)**2))
    # print('gbr poly test rmse = {:0.3f}'.format(rmse))
    #
    # # plot
